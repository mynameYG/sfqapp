			...
			<view class="citys" v-if="!serachCity">
				<view v-for="(city, index) in sortItems" :key="index" v-show="city.isCity" class="citys-row">
					<view class="citys-item-letter" :id="'city-letter-' + (city.name === '#' ? '0' : city.name)">
						<view>{{ city.title }}</view>
						<view v-if="city.nameDesc" class="citys-item-desc">{{ city.nameDesc || '' }}</view>
					</view>
			...
			for (let i = 0; i < this.cityData.length; i++) {
				if (this.cityData[i][this.formatName].indexOf(this.serachCity) !== -1) {
					searchData.push({
						...
						name: this.cityData[i][this.indexName] + "-" + this.cityData[i][this.formatName]
					});
				}
			}
			return searchData;
	...
	methods: {
		...
		initializationCity() {
			...
			for(let index = 0; index < this.cityData.length; index++){
				let code = this.cityData[index][this.indexCode];
				if(arr.indexOf(code) < 0){
					arr.push(code);
					this.handleCity.push({
						name: code,
						nameDesc: this.formatDesc ? this.cityData[index][this.formatDesc] : null,
						title: this.cityData[index][this.indexName],
						isCity: false,
						citys: [],
						forName: 'city-letter-' + code
					});
				}
			}
			
			this.handleCity = this.handleCity.sort(function(a, b){
		...
		buildCityindexs() {
			...
			for (let i = 0; i < this.cityData.length; i++) {
				// 获取首字母
				const cityLetter = this.cityData[i][this.indexCode];
		...
		cityIndexPosition(letter) {
			...
			for(var i = 0; i < this.handleCity.length; i++){
				if(this.handleCity[i].name == letter){
					return i;
				}
			}
			return '';
