			...
			<view class="model-wrap p15">
				<view class="model-box no-mb clearfix">
					...
					<view class="model-item flex flexmid">
						...
						<input name="address" v-model="info.address" type="text" placeholder-class="gray-place" class="model-editText tr flex1 text-ellipsis">
						<text class="model-decorate" @click="getLocation"><text class="iconfont icon-dingwei" style="font-size:25px"></text></text>
					</view>
					<view class="map-wrap">
						<map id="maps" ref="maps" :latitude="latitude" :longitude="longitude" :markers="covers"  scale="13" 
					...
					<view class="model-item">
						...
						<view class="model-editText no-ml heigthAuto">
							<textarea maxlength="-1" name="remark" v-model="info.remark" placeholder="请输入问题描述" placeholder-class="gray-place" class="flex1 model-textarea"></textarea>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getTypes(){
				this.$http.get(`/app/inspection/problemType`).then(res => {
			...
			getLocation() {
				...
				uni.getLocation({
				    ...
				    success: function (res) {
						...
						_self.getCommunity(_self.longitude, _self.latitude)
				    },
					fail() {
						uni.showToast({
							title: '找不到您的位置，请开启定位',
			...
			formSubmit: function(e) {
				...
				this.map.getCenterLocation({
					success: ret => {
						...
						e.detail.value.inspectionId = this.id;
						e.detail.value.problemType = this.problemTypeCode;//类型Code
						e.detail.value.address = this.info.address;
						e.detail.value.mileageStake = this.info.mileageStake;
						e.detail.value.meters = this.info.meters;
						e.detail.value.remark = this.info.remark;
						e.detail.value.reportPersonId = this.$store.state.user.id;//上报人id
						e.detail.value.reportPersonPhone = this.$store.state.user.ext.phone;//上报人phone
						e.detail.value.reportPerson =  this.$store.state.user.nickName
						...
						if (checkRes) {
							...
							this.$http.post('/app/inspection/saveProblem', params).then(formData => {
								uni.showToast({title: "问题上报成功",icon: 'none'});
